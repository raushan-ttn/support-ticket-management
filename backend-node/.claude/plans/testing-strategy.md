...
`NODE_ENV=test` must route to `ttn_stm_test` (verify in `src/config/index.ts`) and activate nodemailer `jsonTransport` and `STORAGE_BACKEND=local`. Job workers are tested by calling their handler functions **directly** (bypass the queue), with `AUTO_CLOSE_DELAY_MS=0`.
...
| `src/jobs/emailWorker.test.ts` | Create | TEST-7 — recipient sets via `jsonTransport` |
| `src/jobs/autoCloseWorker.test.ts` | Create | TEST-8 — four cases via direct handler calls |
...
- `afterAll`: close BullMQ workers/queues, then `pool.end()` and `redis.quit()` to avoid open-handle leaks.
...
| **TEST-7** | Worker unit | `new-ticket` → creator+admin de-duplicated; `comment-notification` excludes author; correct set per role combo; `jsonTransport`, no real mail (FR-10/11) |
| **TEST-8** | Worker unit | (a) assignee comment schedules `jobId=auto-close:{ticketId}`; (b) creator reply removes it; (c) deadline → `CLOSED` via system transition + notification enqueued; (d) last-moment creator reply → FR-12c re-validation → unchanged; `AUTO_CLOSE_DELAY_MS=0` |
...
## Risks
- **Open handles**: BullMQ workers + ioredis + pg must all be closed in `afterAll` or Jest hangs. Run with `--detectOpenHandles` while stabilizing.
...
- **Worker/queue bleed**: auto-close/email tests must use unique `ticketId`s and clear queues between tests to avoid cross-test job leakage.
...
6. Worker tests (TEST-7/8) after notifications + auto-close land
